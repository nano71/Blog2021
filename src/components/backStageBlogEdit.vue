<template>
  <v-row class="white ma-4 pa-4 rounded">
    ...
    <v-col cols="6">
      <v-col cols="6">
        <v-text-field label="博客URL" value="Test"></v-text-field>
      </v-col>
      <v-col cols="6">
        <div class="black06 scale-075">若不编辑，则默认当前时间</div>
        <v-btn color="primary" dark @click.stop="date_dialog = true">
          自定义发表日期
        </v-btn>
      </v-col>
      ...
      <v-dialog v-model="time_dialog" max-width="600">
        <v-card class="pa-4">
          ...
          <v-card-actions class="mt-4">
            <v-btn text color="blue accent-4" @click="time_dialog = false">
              返回日期选择
            </v-btn>
            <v-btn text color="blue accent-4" @click="time_dialog = false">
              确定
